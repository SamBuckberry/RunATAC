...
Tn5 has a ~21 bp sequence preference (Buenrostro *et. al.* 2013, *Nat. Methods*). Here we construct a 21 bp positional weight matrix (PWM) for the Tn5 insertion postions and the flanking 10 bases.   
```{r}
...
```{r}
ins_bias <- calc_ins_bias(ins_pwm = ins_pwm, regions_gr = peaks,
                          genome = Scerevisiae)
```
...
reb1 <- getMatrixByName(JASPAR2016, name=c("CBF1"))
